...
def content_based_recommendations(df, input_course, courses):
	...
	print(temp_sim['course_name'])
	print("Top 5 most dissimilar courses")
	print(temp_dissim['course_name'])

def prep_for_cbr(df, in_course):


	# filter by skills
	skills_avail = ['Programming Skills','Soft Skills', 'Analytical Skills']
	
	
	skills_select =  skills_avail
	# use button to make the update of filtering
	skill_filtered = None
	courses = None
	input_course = "Nothing"
	#if st.sidebar.button("Filter Courses"):
	temp = filter(df, skills_select, 'course_name', 'course_url')
	skill_filtered = df[['course_category','course_name', 'course_url']].reset_index()
	...
	print("**Number of courses:**",
		skill_filtered[skill_filtered['course_category']=='Programming Skills'].shape[0])
	print("**Number of professional degrees:**",
		skill_filtered[skill_filtered['course_category']=='Soft Skills'].shape[0])
	print("**Number of specializations:**",
		skill_filtered[skill_filtered['course_category']=='Analytical Skills'].shape[0])
